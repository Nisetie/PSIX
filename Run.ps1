...
`$dt = get-date;
`$msg = '[' + `$dt.ToString() + '] $hostName. Begin.';
Set-Content -Path "$([System.IO.Path]::Combine($livePath,$hostName + '.txt'))" -Value "`$msg" -Force;
...
try {
	...
	`$dt = get-date;
	`$msg = '[' + `$dt.ToString() + '] ' + `$error[0].ToString();
	Add-Content -Path "$([System.IO.Path]::Combine($livePath,$hostName + '.txt'))" -Value "`$msg" -Force;
	
} finally {

`$dt = get-date;
`$msg = '[' + `$dt.ToString() + '] $hostName. End.';
Add-Content -Path "$([System.IO.Path]::Combine($livePath,$hostName + '.txt'))" -Value "`$msg" -Force;
...
"@;
    ...
    for ($i = 0; $i -lt $hostsFiles.Count; ++$i) {
		...
		[void]$PSInstance.AddParameter('livePath',[System.IO.Path]::Combine($targetPath,"Live"));
