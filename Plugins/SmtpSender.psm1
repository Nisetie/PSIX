...
#$from = [mailaddress]::new("SU_GIVC@nornik.ru");
$from = [mailaddress]::new("su_givc@nornik.ru");
#$to = [mailaddress]::new("SU_GIVC@nornik.ru");
$to = [mailaddress]::new("GorelovNI@nornik.ru");
...
$client = [System.Net.Mail.SmtpClient]::new('owanr.nornik.ru',587);
$client.Credentials = [pscredential]::new("su_givc", (ConvertTo-SecureString -String "SUGIVC908po" -AsPlainText -Force) )
...
if (test-path ([System.IO.Path]::Combine( $liveCatalog, "$($currentHost.HostName)_PrevTriggers"))) {
	...
	#SetContent -path ([System.IO.Path]::Combine($liveCatalog, "$($currentHost.HostName)_PrevTriggers")) '';
} 
...
foreach ($trigger in $triggers) {
	...
	for ($i=0; $i -lt $prevTriggers.Count; $i++) {
		...
		if ($key -eq $prevTrigger.TriggerName)  {
            $newTriggersCount--;
			[void]$prevTriggers.removeat($i); $i--;
