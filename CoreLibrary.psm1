...
class UpdateScript {
	[string]$TemplateName; [string]$ElementName;
	[string]$Script;
...
class HostBase
{
    ...
    [void]Update()  
    {
        ...
        # обработка метрик хоста
        $updateScriptsInvoked = Invoke-Command -Session $session -ScriptBlock $scriptBlock -ArgumentList @(,($this.updateScripts.GetEnumerator()|?{$_.Value.IsLocal -eq $false}|select *));       
        if ($updateScriptsInvoked -eq $null) { return; }
        ...
        $this.updateScripts.GetEnumerator()|?{$_.Value.IsLocal -eq $true}|%{
        ...
        $this.templates.GetEnumerator() | % {
            $template = $_.Value;

            $updateScriptsInvoked = Invoke-Command -Session $session -ScriptBlock $scriptBlock -ArgumentList @(,($template.updateScripts.GetEnumerator()|?{$_.Value.IsLocal -eq $false}|select *));       
            if ($updateScriptsInvoked -eq $null) { return; }
                if ($updateScriptsInvoked -is [System.Collections.IEnumerable]) { 
            ...
            $template.updateScripts.GetEnumerator()|?{$_.Value.IsLocal -eq $true}|%{
                $updateScript = $_.Value;
                $sw.Restart();
                $updateScript.CurrentValue = [scriptblock]::Create($updateScript.Script).InvokeReturnAsIs($null);
                $updateScript.UpdateDelta = $sw.Elapsed;
...
function ParseUpdatesCatalog([MetaModel]$info,[string]$className,[string]$path) {
    $updates = Get-ChildItem -Path $path -File -Filter "*.ps1" | ? { $_.Name -like "*.ps1" -or $_.Name -like "*.sh"  };
    $id = 0;
    $initBody = '';
...
`$updateScript.Script = {$scriptText}.ToString();
